...
class SearchBar extends Component {
  ...
  render() {
    return (
      <div className="row"><div className="search-bar col-md-8 col-lg-6">
        <p>Find <em>{this.state.term}</em> on YouTube</p>
        <div className="input-group">
          <input
            className="form-control"
            value={this.state.term}
            onChange={event => this.onInputChange(event.target.value)}/>
          <span className="input-group-btn"><button className="btn btn-secondary" type="button">Search YouTube</button>
          </span>
        </div>
      </div></div>
    );
